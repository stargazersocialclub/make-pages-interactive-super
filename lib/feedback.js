...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '      <button id="cf-move-toggle" class="cf-btn" title="Move element (M)">↕ move <span class="cf-kbd-hint">M</span></button>',
      ...
      '    </div>',
      '    <p class="cf-hint"><strong>Highlight text</strong> to comment on a selection. <strong>Double-click text</strong> to edit it in place — toolbar has bold/italic, lists, links, alignment, case, font, color, border, radius, padding. <strong>Double-click an image</strong> to drag its corners and resize (border + radius stay available). <strong>Shift</strong>+<strong>double-click any element</strong> to resize it as a container (any div, section, card). Press <kbd>E</kbd> to select elements (then comment or <strong>delete</strong>), <kbd>M</kbd> to drag-reorder, <kbd>G</kbd> for a general note. Hold <kbd>Alt</kbd> + drag a rectangle to <strong>snapshot a region</strong>. Toggle a 24 px grid + snap from the panel. Press <kbd>?</kbd> for the full guide. <kbd>Esc</kbd> cancels.</p>',
      '  </div>',
      ...
      '          <li>Hold <kbd>Shift</kbd> + <strong>double-click any element</strong> to resize it as a container — pick a card, section, or div and drag its corners.</li>',
      ...
      '        <ul>',
      '          <li>Row 1: <strong>B</strong> / <strong>I</strong> / bullet / numbered / <strong>link</strong> / unlink / align / case.</li>',
      ...
      '        <ul>',
      '          <li>Every queued change drops a gold marker on its element — <strong>✎</strong> for text/style edits, <strong>💬</strong> for comments, <strong>↕</strong> for moves. Hover to outline the target.</li>',
  ...
  // Drop a stuck-armed snapshot state. Called from every mode entry so the
  // user can't end up with both snapshot and (e.g.) move armed from a stale
  // Alt-down that the focus / blur sequence didn't surface.
  ...
  function toggleMoveMode() {
    ...
    if (btn) {
      ...
      btn.innerHTML = moveMode
        ? '✓ moving <span class="cf-kbd-hint">M</span>'
        : '↕ move <span class="cf-kbd-hint">M</span>';
    }
  ...
  function onSnapshotPointerDown(e) {
    if (!snapshotAltHeld) return;
    // Bail if another mode latched between Alt-keydown and pointerdown — Alt
    // is a chord modifier, and double-firing pointerdown on two handlers
    // would start both a snapshot and (e.g.) a move drag.
    if (editingEl || elementMode || moveMode || moveDragging) return;
    if (insideOurUI(e.target)) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  // Shift + double-click entry point — finds the precise element the user
  // pointed at so any block can be resized as a "container," not just IMG /
  // VIDEO / paragraph. Walks one step up from a text node, refuses to
  // operate on body/html/insideOurUI.
  function findContainerForResize(node) {
    let el = node && node.nodeType === 3 ? node.parentElement : node;
  ...
  // Resize-only edit for arbitrary containers — entered via Shift + double-
  // click. Same shell as startImageEdit but works on any DIV / SECTION /
  // header / card / etc. Keeps the font / color / bg row visible since
  // containers commonly want bg/border/padding tuning. No contenteditable —
  // text editing is what plain double-click is for.
  function startContainerEdit(el) {
    if (editingEl) return;
    if (elementMode) return;
    clearSnapshotArm();
    editingEl = el;
    ...
    buildFontFamilyOptions();
    populateStyleControls(el);
    el.classList.add("cf-editing-target", "cf-editing-container");
    document.body.classList.add("cf-container-edit-mode");
    const label = document.querySelector(".cf-edit-toolbar-label");
    if (label) {
      if (!label.dataset.cfDefaultLabel) label.dataset.cfDefaultLabel = label.textContent;
      label.textContent = "editing container — " + (el.tagName.toLowerCase());
    }
    hideTextPopup();
    positionEditToolbar();
    $("cf-edit-toolbar").classList.add("cf-visible");
  ...
  function exitEditMode(keepVisualEdit) {
    ...
    editingEl.classList.remove("cf-editing-target", "cf-editing-image", "cf-editing-container");
    document.body.classList.remove("cf-image-edit-mode", "cf-container-edit-mode");
  ...
  function cleanOuterHtml(el) {
    ...
    clone.classList.remove("cf-editing-container");
  ...
  function onDblClick(e) {
    ...
    // Shift held: resize the exact element under the cursor as a container
    // regardless of whether it's text-bearing or an image. Lets the user
    // pick precisely (drag the corner of a card, a section, the row above
    // the photos, etc.) without the text-edit / image-edit dispatch deciding
    // for them.
    if (e.shiftKey) {
      const container = findContainerForResize(e.target);
      if (container) {
        e.preventDefault();
        startContainerEdit(container);
        return;
      }
    }
  ...
  function renderPending() {
    ...
    pending.forEach((c) => {
      ...
      if (c.type === "general") {
        ...
        quote.innerHTML =
          `<div class="cf-pq-label">move · ${escapeHtml(meTag)}</div>` +
          (meShort ? `<div class="cf-pq-text">${escapeHtml(meShort)}</div>` : "") +
          `<div class="cf-pq-meta">${escapeHtml(parentLabel)}: position ${escapeHtml(String(fromIdx))} → ${escapeHtml(String(toIdx))}</div>`;
      } else if (c.type === "snapshot") {
        const r = c.region || {};
  ...
  // Removing one move when other pending moves share its parent: unwind every
  // move on that parent in reverse-creation order (each back to its from.index),
  // then replay the survivors in creation order at their to.index. Net effect:
  // the parent ends up looking like the surviving move set, with c gone.
  function revertMoveAndReplay(c) {
    if (!c.parent) return;
    let parent = null;
    if (c.parent.id) parent = document.getElementById(c.parent.id);
    if (!parent && c.parent.selector) {
    ...
    const samePar = (m) => {
      ...
      if (m.parent.id && c.parent.id && m.parent.id === c.parent.id) return true;
      if (m.parent.selector && c.parent.selector && m.parent.selector === c.parent.selector) return true;
      return false;
    };
    const allOnParent = pending
      .filter(p => p.type === "move" && samePar(p))
      .sort((a, b) => (a.created_at || "").localeCompare(b.created_at || ""));
    const insertAt = (el, idx) => {
      const others = Array.from(parent.children).filter(x => x !== el);
      const i = Math.max(0, Math.min(idx, others.length));
      if (i >= others.length) parent.appendChild(el);
      else parent.insertBefore(el, others[i]);
    };
    // Step 1 — undo in reverse-creation order
    for (let i = allOnParent.length - 1; i >= 0; i--) {
      const m = allOnParent[i];
      const el = findElementByAnchorInfo(m.element);
      if (!el || el.parentElement !== parent) continue;
      insertAt(el, m.from && m.from.index != null ? m.from.index : 0);
    }
    // Step 2 — replay survivors in creation order
    const survivors = allOnParent.filter(p => p.id !== c.id);
    for (const m of survivors) {
      const el = findElementByAnchorInfo(m.element);
      if (!el || el.parentElement !== parent) continue;
      insertAt(el, m.to && m.to.index != null ? m.to.index : 0);
    }
  }
  ...
  function clearAllPending() {
    ...
    pending = [];
    // Moves need group-aware unwind: revertMoveAndReplay reads the live
    // pending list to find survivors and replays them, which is wrong when
    // we want to revert ALL of them. Do the move-stack unwind inline here,
    // per parent, in reverse-creation order, with no replay step.
    const movesByParent = new Map();
    toRevert.forEach((c) => {
      if (c.type !== "move" || !c.parent) return;
      const key = c.parent.id || c.parent.selector || "?";
      if (!movesByParent.has(key)) movesByParent.set(key, []);
      movesByParent.get(key).push(c);
    });
    movesByParent.forEach((moves) => {
      moves.sort((a, b) => (a.created_at || "").localeCompare(b.created_at || ""));
      const c0 = moves[0];
      let parent = null;
      if (c0.parent.id) parent = document.getElementById(c0.parent.id);
      if (!parent && c0.parent.selector) {
        try { parent = document.querySelector(c0.parent.selector); } catch (e) {}
      }
      if (!parent) return;
      for (let i = moves.length - 1; i >= 0; i--) {
        ...
        if (!el || el.parentElement !== parent) continue;
        const others = Array.from(parent.children).filter((x) => x !== el);
        const idx = Math.max(0, Math.min(m.from && m.from.index != null ? m.from.index : 0, others.length));
        if (idx >= others.length) parent.appendChild(el);
        else parent.insertBefore(el, others[idx]);
      }
    });
    // text-edit / delete use the existing per-comment revert path.
    toRevert.filter((c) => c.type !== "move").forEach((c) => {
      try { revertCommentVisual(c); } catch (e) { console.warn("revert failed for", c.id, e); }
    });
    saveLS();
    renderPending();
  ...
  function bindEvents() {
    ...
    ["cf-edit-bold", "cf-edit-italic", "cf-edit-ul", "cf-edit-ol", "cf-edit-link", "cf-edit-unlink",
