...
(function () {
  ...
  function applyBgState() {
    ...
    if (bgState.imgUrl) layers.push(`url("${bgState.imgUrl.replace(/"/g, '\\"')}")`);
    ...
    // bg size / position / repeat: only write when an image is in play AND the
    // value has actually changed from the original. Skip for gradient-only or
    // bg-color-only edits — otherwise we'd override sensible CSS defaults.
    if (bgState.imgUrl) {
      if (bgState.imgSize !== orig.imgSize) {
        el.style.setProperty("background-size", bgState.imgSize, "important");
      }
      if (bgState.imgPos !== orig.imgPos) {
        el.style.setProperty("background-position", bgState.imgPos, "important");
      }
      if (!orig.imgUrl) {
  ...
  function renderBgStops() {
    ...
    bgState.stops.forEach((stop, idx) => {
      ...
      alphaInput.className = "cf-style-num cf-num-spin cf-bg-stop-alpha";
      alphaInput.title = "Stop alpha 0–100%";
      alphaInput.addEventListener("input", (e) => {
        stop.alpha = Math.max(0, Math.min(100, parseFloat(e.target.value) || 0)) / 100;
        swatch.style.background = rgbaString(stop.color, stop.alpha);
        applyBgState();
      });
  ...
  function wireBgToolbar() {
    ...
    $("cf-bg-stop-add").addEventListener("click", (e) => {
      ...
      bgState.stops.push({
        color: { ...(last.color) },
        alpha: last.alpha != null ? last.alpha : 1,
  ...
  function computePanLimits(el) {
    ...
    const zoom = zMatch ? parseFloat(zMatch[1]) : 1;
    // object-fit:cover scales the image by max(fw/nw, fh/nh) to fill the frame.
    // Effective on-screen image size = nw|nh * fitScale * zoom.
  ...
  function renderPending() {
    ...
    pending.forEach((c) => {
      ...
      if (c.type === "general") {
        ...
        quote.innerHTML =
          `<div class="cf-pq-label">section reorder ${arrow}</div>` +
      ...
      del.addEventListener("click", (e) => {
        ...
        if (c.type === "text-edit" || c.type === "delete" || c.type === "section-reorder") revertCommentVisual(c);
  ...
  function revertCommentVisual(c) {
    if (c.type === "text-edit") {
      ...
      // Undo by swapping back — direction flips.
      if (c.direction === "up") moved.parentNode.insertBefore(swapped, moved);
      else moved.parentNode.insertBefore(moved, swapped);
      updateReorderButtonStates();
    }
  }
      ...
      if (editingEl && document.body.classList.contains("cf-image-edit-mode") &&
        ...
        if (e.key === "+" || e.key === "=" || e.key === "-") {
          ...
          if (scale === 1) {
            ...
            editingEl.style.setProperty("transform", `scale(${scale})`, "important");
            // clip-path: inset() in pixels is in the element's pre-transform local
            // space, so a compensating inset clips the rendered output back to the
            // original layout box. Formula: inset = side * (1 - 1/scale) / 2.
            // Net effect — zooming in scales the image content visible inside the
            // frame WITHOUT growing the visible box. At scale 1, clip-path is
            // removed so it doesn't leave a stale clip on the source.
            const fw = editingEl.offsetWidth || 0;
            const fh = editingEl.offsetHeight || 0;
            const inX = (fw * (1 - 1 / scale)) / 2;
            const inY = (fh * (1 - 1 / scale)) / 2;
            editingEl.style.setProperty("clip-path", `inset(${inY.toFixed(2)}px ${inX.toFixed(2)}px)`, "important");
          }
          editingEl.style.setProperty("transform-origin", "center", "important");
          // After zoom changes, re-clamp the current pan against the new effective
          // image overflow so the image edge can't separate from the frame edge.
