...
(function () {
  ...
  // removed. text-edit restores innerHTML + style attribute from the captured
  // originals; move re-inserts the moved element at its from-position.
  // selection / elements / general / snapshot don't mutate the page, so they
  // need no revert.
  function revertCommentVisual(c) {
    if (c.type === "text-edit") {
      ...
      // The style attribute lives on the outer_html (not innerHTML). Pull it
      // out so font / color / width / margins / borders all snap back.
      if (c.original_outer_html) {
        const m = c.original_outer_html.match(/<[^>]*\sstyle\s*=\s*"([^"]*)"/i) ||
                  c.original_outer_html.match(/<[^>]*\sstyle\s*=\s*'([^']*)'/i);
        if (m) t.setAttribute("style", m[1]);
        else t.removeAttribute("style");
      }
    } else if (c.type === "move") {
      const t = findElementByAnchorInfo(c.element);
      if (!t || !c.parent) return;
      let parent = null;
      ...
      if (!parent) return;
      const nextAnchor = c.from && c.from.next_anchor;
      const nextSibling = nextAnchor ? findChildByAnchor(parent, nextAnchor) : null;
      if (nextSibling) parent.insertBefore(t, nextSibling);
      else parent.appendChild(t);
    }
  }
  function findChildByAnchor(parent, anchor) {
    if (!anchor) return null;
    if (anchor.cf_id) {
      const el = parent.querySelector('[data-cf-id="' + anchor.cf_id + '"]');
      if (el && el.parentElement === parent) return el;
    }
    if (anchor.id) {
      const el = document.getElementById(anchor.id);
      if (el && el.parentElement === parent) return el;
    }
    if (anchor.data_cf_change) {
      const el = parent.querySelector('[data-cf-change="' + anchor.data_cf_change + '"]');
      if (el && el.parentElement === parent) return el;
    }
    if (anchor.text_snippet) {
      const needle = anchor.text_snippet.slice(0, 30);
      for (const child of parent.children) {
        const t = (child.innerText || child.textContent || "").replace(/\s+/g, " ").trim();
        if (t.startsWith(needle)) return child;
      }
    }
    return null;
  }
  ...
  function bindEvents() {
    ...
    window.addEventListener("resize", () => {
      ...
      clearTimeout(window._cfMarkerResizeTimer);
      window._cfMarkerResizeTimer = setTimeout(refreshPendingEditMarkers, 120);
    });
