...
(function () {
  ...
  function populateBgToolbar() {
    ...
    renderBgStops();
    // Show ::before texture name + opacity slider when texture is detected.
    const texRow = $("cf-bg-row-tex");
    const texName = $("cf-bg-tex-name");
    if (texRow && texName && bgEditingEl) {
      try {
        ...
        if (m) {
          ...
          texName.title = m[1];
          texRow.style.display = "";
          // Init opacity slider from --tex-opacity inline custom property.
          ...
          // Show URL override row; pre-fill if --tex-url already set.
          const texUrlRow = $("cf-bg-row-tex-url");
          const texUrlInput = $("cf-bg-tex-url-input");
          if (texUrlRow && texUrlInput) {
            texUrlRow.style.display = "";
            const rawUrl = bgEditingEl.style.getPropertyValue("--tex-url").trim();
            // --tex-url is stored as url("...") — extract just the bare URL for display.
            const urlMatch = rawUrl.match(/url\(["']?([^"')]+)["']?\)/);
            texUrlInput.value = urlMatch ? urlMatch[1] : "";
          }
        } else {
          texRow.style.display = "none";
          const texUrlRow = $("cf-bg-row-tex-url");
          if (texUrlRow) texUrlRow.style.display = "none";
        }
      } catch (e) {
        texRow.style.display = "none";
      }
  ...
  function wireBgToolbar() {
    ...
    $("cf-bg-tex-clear").addEventListener("click", (e) => {
      ...
      // Also clean up --tex-opacity inline if a class was removed (no longer needed).
      if (found) bgEditingEl.style.removeProperty("--tex-opacity");
      const texRow = $("cf-bg-row-tex");
    ...
    $("cf-bg-tex-url-apply").addEventListener("click", (e) => {
      ...
      // Update the filename chip to reflect the new URL.
      const texName = $("cf-bg-tex-name");
      if (texName) {
        const parts = url.split("/");
        texName.textContent = parts[parts.length - 1] || url;
        texName.title = url;
      }
    });
