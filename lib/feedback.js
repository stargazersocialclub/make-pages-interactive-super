...
(function () {
  ...
  // Per-comment revert for clearAllPending. text-edit and delete reuse the
  // existing revertCommentVisual paths; move is inlined here so it can run
  // without revertMoveAndReplay's pending-survivors lookup (we already nuked
  // the queue — there are no survivors). selection / elements / general /
  // snapshot don't mutate the page, so they're no-ops.
  function clearAllRevertOne(c) {
    if (c.type === "text-edit" || c.type === "delete") {
      revertCommentVisual(c);
      return;
    }
    if (c.type !== "move" || !c.parent || !c.element) return;
    let parent = null;
    if (c.parent.id) parent = document.getElementById(c.parent.id);
    if (!parent && c.parent.selector) {
      try { parent = document.querySelector(c.parent.selector); } catch (e) {}
    ...
    if (!parent) return;
    const el = findElementByAnchorInfo(c.element);
    if (!el || el.parentElement !== parent) return;
    const others = Array.from(parent.children).filter((x) => x !== el);
    const idx = Math.max(0, Math.min(c.from && c.from.index != null ? c.from.index : 0, others.length));
