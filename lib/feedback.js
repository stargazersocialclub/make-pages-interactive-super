...
(function () {
  ...
  function sqsApplySelected() {
    ...
    // Group style-block changes per block so multiple rewrites in the
    // same <style> share one text-edit entry.
    ...
    findings.h1.forEach((f, i) => {
      ...
      const new_outer_html = fresh.outerHTML;
      pending.push({
        id: "c-sqs-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
        type: "text-edit",
        created_at: new Date().toISOString(),
        comment: "Squarespace audit: rewrote a duplicate <h1> as <" + newTag + ">",
        elements: [anchorInfo(fresh)],
        original_text: fresh.innerText || "",
        new_text: fresh.innerText || "",
        original_html: fresh.innerHTML,
        new_html: fresh.innerHTML,
        original_outer_html,
        new_outer_html,
      });
      queued++;
    ...
    // CHECK 4 — z-index in nav range: rewrite value in <style> text
    findings.zindex.forEach((f, i) => {
      if (!checked("zindex", i)) return;
      const cur = getBlockText(f.blockIdx);
      // Replace the EXACT match string at its captured offset to avoid
      // collateral hits on identical values elsewhere in the block.
    ...
    // CHECK 5 — --tweak-* vars: rename to --ssc-* in <style> text
    findings.tweakvars.forEach((f, i) => {
      if (!checked("tweakvars", i)) return;
      const cur = getBlockText(f.blockIdx);
      // Rename all occurrences in this block — variable usage matters too,
    ...
    findings.fonts.forEach((f, i) => {
      ...
      const cur = getBlockText(f.blockIdx);
      // Match the @import line (already captured in f.match) and remove
      // it cleanly along with leading whitespace.
      const re = new RegExp("\\s*" + f.match.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g");
      setBlockText(f.blockIdx, cur.replace(re, ""));
    });

    // CHECK 7 — missing box-model reset: prepend a reset rule to the
    // first block that references the affected root. If no block does,
    // prepend to block 0.
    ...
    // CHECK 8 — inline styles missing !important (contextual; only present
    // when no scoped root was detected)
