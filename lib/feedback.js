...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '  <div class="cf-edit-style-wrap">',
      '    <button id="cf-edit-style-toggle" class="cf-btn cf-btn-small cf-edit-fmt" title="Element style" aria-label="Element style">Aa<span class="cf-style-chev">▾</span></button>',
      '    <div id="cf-edit-style-panel" class="cf-edit-style-panel" role="dialog" aria-label="Element style">',
      '      <div class="cf-style-inner">',
      '        <div class="cf-style-row"><label class="cf-style-lbl">font</label><select id="cf-edit-font-family"></select><input type="number" id="cf-edit-font-size" min="8" max="120" step="0.5" class="cf-style-num" title="Font size (px)"></div>',
      '        <div class="cf-style-row"><label class="cf-style-lbl">color</label><input type="color" id="cf-edit-color" class="cf-color-input"><label class="cf-style-lbl" style="margin-left:10px;">bg</label><input type="color" id="cf-edit-bg" class="cf-color-input"></div>',
      '        <div class="cf-style-row"><label class="cf-style-lbl">border</label><input type="range" id="cf-edit-border-w" min="0" max="10" value="0" class="cf-style-range" title="Border thickness"><input type="color" id="cf-edit-border-c" class="cf-color-input" title="Border color"></div>',
      '        <div class="cf-style-row"><label class="cf-style-lbl">radius</label><input type="range" id="cf-edit-radius" min="0" max="40" value="0" class="cf-style-range" title="Border radius"></div>',
      '        <div class="cf-style-row" style="justify-content:flex-end;"><button id="cf-edit-style-reset" class="cf-btn cf-btn-small">reset styles</button></div>',
      '      </div>',
      '    </div>',
      '  </div>',
  ...
  function exitEditMode(keepVisualEdit) {
    if (!editingEl) return;
    if (!keepVisualEdit) editingEl.innerHTML = editingOriginalHtml;
    ...
    // Close any open style panels
    document.querySelectorAll(".cf-edit-style-wrap.cf-style-open")
      .forEach((w) => w.classList.remove("cf-style-open"));
  ...
  // Decide between auto-submitting (small diff) and opening the confirm dialog
  // (substantive change). Called from both ⌘↵ and the "confirm" toolbar button.
  function confirmOrAutoSubmit() {
    ...
    // Style-only changes always go straight through (the confirm dialog has
    // nothing useful to show for them).
    if (!textChanged) { submitTextEdit(); return; }
    const d = simpleWordDiff(editingOriginalText, newText);
    const changeSize = Math.max(d.removed.length, d.added.length);
    if (changeSize > 0 && changeSize <= SKIP_CONFIRM_DIFF_THRESHOLD) {
      submitTextEdit();
      return;
    }
    openEditConfirm();
  ...
  function applyInlineStyle(prop, value) {
    ...
    else editingEl.style.setProperty(prop, value);
  }
  function applyBorder() {
    ...
    if (w === 0) {
      ...
      editingEl.style.setProperty("border", w + "px solid " + c);
    }
  }
  function applyRadius() {
    ...
    else editingEl.style.setProperty("border-radius", r + "px");
  }
  ...
  function renderPending() {
    ...
    pending.forEach((c) => {
      ...
      if (c.type === "general") {
        ...
        quote.innerHTML =
          `<div style="font-size:10px;text-transform:uppercase;letter-spacing:.14em;color:#b14000;font-weight:600;margin-bottom:3px;">text edit · ${escapeHtml(tag)}</div>` +
          `<div style="background:#f7e9e3;color:#6a2a1a;padding:3px 6px;border-radius:3px;margin-bottom:3px;font-style:normal;"><s>${escapeHtml(beforeShort)}</s></div>` +
          `<div style="background:#e4f2e1;color:#1f5022;padding:3px 6px;border-radius:3px;font-style:normal;">${escapeHtml(afterShort)}</div>`;
      } else {
  ...
  function bindEvents() {
    ...
    ["cf-edit-bold", "cf-edit-italic"].forEach((id) => {
    ...
    // Style panel: click toggle (sticky open), hover preview (handled in CSS)
    $("cf-edit-style-toggle").addEventListener("mousedown", (e) => e.preventDefault());
    $("cf-edit-style-toggle").addEventListener("click", (e) => {
      e.stopPropagation();
      $("cf-edit-style-toggle").parentElement.classList.toggle("cf-style-open");
    });
    // Click outside the style panel closes any sticky-open state
    document.addEventListener("click", (e) => {
      const wrap = e.target.closest(".cf-edit-style-wrap");
      if (!wrap) {
        document.querySelectorAll(".cf-edit-style-wrap.cf-style-open")
          .forEach((w) => w.classList.remove("cf-style-open"));
      }
    ...
    // Keep the toolbar / confirm dialog glued to the element as the user scrolls or resizes
    window.addEventListener("scroll", () => {
      if (editingEl) positionEditToolbar();
