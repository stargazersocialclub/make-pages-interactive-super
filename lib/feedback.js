...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    <input type="number" id="cf-edit-border-w" min="0" max="20" step="0.5" value="0" class="cf-style-num" title="Border weight (px)">',
      '    <input type="color" id="cf-edit-border-c" class="cf-color-input" title="Border color">',
      '    <label class="cf-style-lbl">radius</label>',
      '    <input type="number" id="cf-edit-radius" min="0" max="80" step="1" value="0" class="cf-style-num" title="Border radius (px)">',
  ...
  function onResizeMove(e) {
    ...
    if (resizeStart.snapEdges) {
      const xs = gridSnap ? resizeStart.snapEdges.xs.concat(nearestGridLines(resizeStart.left + dx, GRID_SIZE_PX)) : resizeStart.snapEdges.xs;
      const ys = gridSnap ? resizeStart.snapEdges.ys.concat(nearestGridLines(resizeStart.top + dy, GRID_SIZE_PX))  : resizeStart.snapEdges.ys;
      if (resizeStart.cornerX === "r") {
        const snapped = snapToward(resizeStart.left + resizeStart.w + dx, xs);
        dx = snapped - resizeStart.left - resizeStart.w;
      } else {
        const snapped = snapToward(resizeStart.left + dx, xs);
        dx = snapped - resizeStart.left;
      }
      if (resizeStart.cornerY === "b") {
        const snapped = snapToward(resizeStart.top + resizeStart.h + dy, ys);
        dy = snapped - resizeStart.top - resizeStart.h;
      } else {
        const snapped = snapToward(resizeStart.top + dy, ys);
        dy = snapped - resizeStart.top;
      }
    }
