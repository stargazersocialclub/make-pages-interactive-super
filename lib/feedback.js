...
(function () {
  ...
  // Drag-to-move state (⤢ move button in element popup)
  let dragState = null; // { x, y, origMarginTop, origMarginLeft, dragArmed }
  ...
  function getCleanPageHtml() {
    ...
    const SESSION_CLASSES = [
      "cf-editing-target", "cf-editing-image", "cf-bg-editing-target",
  ...
  // Capture the window state at submit time so the agent can tell whether the
  // user was looking at desktop, tablet, or mobile when they wrote a comment —
  // without it, layout feedback like "this is broken" is ambiguous.
  ...
  // Walk up from el.parentElement to find the next commentable ancestor.
  // Used by Alt+click and the ↑ parent popup button.
  function findParentCommentable(el) {
    if (!el) return null;
    let cur = el.parentElement;
    while (cur && cur !== document.body && cur !== document.documentElement) {
      if (insideOurUI(cur)) return null;
      if (cur.dataset && cur.dataset.cfId) return cur;
      if (isCommentable(cur)) return cur;
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '          <tr><td><kbd>C</kbd></td><td>smart comment</td><td><kbd>L</kbd></td><td>copy HTML</td></tr>',
      ...
      '  <button id="cf-elem-popup-parent"  class="cf-btn cf-btn-small" title="Select parent element (or Alt+click)">↑ parent</button>',
      '  <button id="cf-elem-popup-resize"  class="cf-btn cf-btn-small" title="Open resize handles on this element">↕ resize</button>',
      '  <button id="cf-elem-popup-move"    class="cf-btn cf-btn-small" title="Drag to reposition this element">⤢ move</button>',
      ...
      '<div id="cf-drag-handle" class="cf-drag-handle" title="Drag to reposition" aria-label="Move element">✥</div>',
      '<div id="cf-edit-toolbar" class="cf-edit-toolbar" role="toolbar" aria-label="Edit text">',
      // Row 0 — action: label + conflict chip on left, 💬 cancel confirm on right
      '  <div class="cf-edit-toolbar-row cf-action-row">',
      ...
      '    <span class="cf-tb-spacer" aria-hidden="true"></span>',
      ...
      '  </div>',
      // Row 1 — format buttons (image-only buttons hidden in text mode via CSS)
      '  <div class="cf-edit-toolbar-row cf-format-row">',
      ...
      '    <button id="cf-edit-img-mirror" class="cf-btn cf-btn-small cf-img-only" title="Mirror horizontally" aria-label="Mirror image horizontally"><svg width="14" height="12" viewBox="0 0 14 12" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><line x1="7" y1="1" x2="7" y2="11" stroke-dasharray="1.6 1.4"/><path d="M5 3.6 L2 6 L5 8.4"/><path d="M9 3.6 L12 6 L9 8.4"/></svg></button>',
      ...
      // right → left. Hidden in image-edit mode via .cf-edit-fmt.
      ...
      '    </button>',
      // Vertical alignment cycle — top → middle → bottom.
      ...
      '    </button>',
      // Case cycle — UPPERCASE → lowercase → Title Case.
      ...
      '    </button>',
      '  </div>',
      // Row 2 — font / size / line-height (hidden in image-edit mode)
      ...
      '  </div>',
      // Row 3 — color / bg / text-fx / reset (hidden in image-edit mode)
      '  <div class="cf-edit-toolbar-row cf-style-row cf-style-row-text">',
      ...
      '    <label class="cf-style-lbl">txt fx</label>',
      '    <select id="cf-edit-effect" class="cf-bg-select" title="Text effect — applied to the type itself">',
      '      <option value="">none</option>',
      '      <option value="cf-fx-glow">glow</option>',
      '      <option value="cf-fx-gradient">gradient</option>',
      '      <option value="cf-fx-engraved">engraved</option>',
      '      <option value="cf-fx-drop">shadow</option>',
      '      <option value="cf-fx-underline-draw">underline</option>',
      '    </select>',
      '    <button id="cf-edit-style-reset" class="cf-btn cf-btn-small" title="Reset font/color/bg/border overrides">reset</button>',
      '  </div>',
      // Row 4 — border / radius / padding
      ...
      '  </div>',
      // Row 5 — opacity / box-fx / paintbrush / help
      '  <div class="cf-edit-toolbar-row cf-style-row cf-style-row-border">',
      ...
      '    <label class="cf-style-lbl">box fx</label>',
      '    <select id="cf-edit-boxfx" class="cf-bg-select" title="Box effect — applied to the border / surround">',
      '      <option value="">none</option>',
      '      <option value="cf-bxfx-glow">glow</option>',
      '      <option value="cf-bxfx-shadow">shadow</option>',
      '      <option value="cf-bxfx-inset-glow">inset glow</option>',
      '    </select>',
      ...
      '  <button type="button" id="cf-color-popover-transparent" class="cf-btn cf-btn-small" style="display:none">⊘ transparent</button>',
      ...
      '    <button id="cf-bg-img-pos" class="cf-btn cf-btn-small" data-cf-bgpos="center" title="Click to cycle keyword position. Scroll/wheel over the bg to fine-tune (calc-pair offsets).">pos: center</button>',
      '  </div>',
      // Texture indicator row — shows ::before filename + live opacity slider.
      // Opacity writes --tex-opacity onto the element inline so the agent can
      // apply it to source; the ::before rule reads it via var(--tex-opacity,1).
      '  <div class="cf-bg-row cf-bg-row-tex" id="cf-bg-row-tex" style="display:none">',
      '    <label class="cf-bg-lbl" title="Background texture via CSS ::before pseudo-element">texture</label>',
      '    <span id="cf-bg-tex-name" class="cf-bg-tex-name" title="">—</span>',
      '    <label class="cf-bg-lbl cf-bg-tex-op-lbl">opacity</label>',
      '    <input type="range" id="cf-bg-tex-opacity" class="cf-bg-tex-opacity-range" min="0" max="1" step="0.05" value="1" title="Texture opacity (0 = transparent, 1 = full)">',
      '    <span id="cf-bg-tex-opacity-val" class="cf-bg-tex-opacity-val">1.00</span>',
      '    <button id="cf-bg-tex-clear" class="cf-btn cf-btn-small cf-bg-tex-clear-btn" title="Remove texture class from this section">clear</button>',
      '  </div>',
      // Texture URL override row — paste SQ CDN URL to replace the local file path.
      // Writes --tex-url: url("...") onto the element inline; the ::before rule
      // reads it via var(--tex-url, url("./local-fallback.png")).
      '  <div class="cf-bg-row cf-bg-row-tex-url" id="cf-bg-row-tex-url" style="display:none">',
      '    <label class="cf-bg-lbl">sq url</label>',
      '    <input type="text" id="cf-bg-tex-url-input" class="cf-bg-url-input" placeholder="https://images.squarespace-cdn.com/…" title="Paste SQ CDN URL to overwrite the local texture path">',
      '    <button id="cf-bg-tex-url-apply" class="cf-btn cf-btn-small" title="Apply — writes --tex-url onto this element">apply</button>',
      '    <button id="cf-bg-tex-url-clear" class="cf-btn cf-btn-small" title="Revert to local file path (removes --tex-url)">revert</button>',
      '  </div>',
      // Note row — optional comment that rides along with the bg edit. Empty by
      // default; when filled, it lands in the pending entry as `comment`.
      '  <div class="cf-bg-row cf-bg-row-note">',
      '    <label class="cf-bg-lbl" for="cf-bg-note">note</label>',
      '    <input type="text" id="cf-bg-note" class="cf-bg-url-input" placeholder="(optional) note for Claude about this bg change" title="Optional comment that will land with this bg edit">',
      '  </div>',
      '</div>',
  ...
  function onElemClick(e) {
    ...
    // Alt+click: step up one level to select the parent commentable element.
    const base = findCommentableAncestor(e.target) || e.target;
    const el = e.altKey ? (findParentCommentable(base) || base) : base;
  ...
  function showElemPopup(nearEl) {
    ...
    if (bgBtn) bgBtn.style.display = single ? "" : "none";
    const resizeBtn = $("cf-elem-popup-resize");
    if (resizeBtn) resizeBtn.style.display = single ? "" : "none";
    const moveBtn = $("cf-elem-popup-move");
    if (moveBtn) moveBtn.style.display = single ? "" : "none";
    const parentBtn = $("cf-elem-popup-parent");
    if (parentBtn) {
      const last = selectedElements[selectedElements.length - 1];
      parentBtn.style.display = (last && findParentCommentable(last)) ? "" : "none";
    }
    popup.classList.add("cf-visible");
  }
  ...
  function startTextEdit(el) {
    ...
    sel.removeAllRanges();
    sel.addRange(range);
    const _tbLabel = document.querySelector(".cf-edit-toolbar-label");
    if (_tbLabel) {
      if (!_tbLabel.dataset.cfDefaultLabel) _tbLabel.dataset.cfDefaultLabel = _tbLabel.textContent;
      _tbLabel.textContent = elementShortLabel(el);
    }
    positionEditToolbar();
  ...
  function startImageEdit(el) {
    ...
    if (label) {
      ...
      label.textContent = elementShortLabel(el);
    }
    hideTextPopup();
    positionEditToolbar();
    $("cf-edit-toolbar").classList.add("cf-visible");
  ...
  // ---- Drag-to-move (⤢ move button) ----
  function positionDragHandle() {
    if (!editingEl) return;
    const h = $("cf-drag-handle");
    if (!h) return;
    const r = editingEl.getBoundingClientRect();
    h.style.left = (r.left + r.width  / 2 - 18) + "px";
    h.style.top  = (r.top  + r.height / 2 - 18) + "px";
  }
  ...
  function startDragEdit(el) {
    ...
    buildFontFamilyOptions();
    populateStyleControls(el);
    el.classList.add("cf-editing-target", "cf-editing-drag");
    document.body.classList.add("cf-image-edit-mode");
    const label = document.querySelector(".cf-edit-toolbar-label");
    if (label) {
      if (!label.dataset.cfDefaultLabel) label.dataset.cfDefaultLabel = label.textContent;
      label.textContent = "move: " + elementShortLabel(el);
    }
    hideTextPopup();
    positionEditToolbar();
    $("cf-edit-toolbar").classList.add("cf-visible");
  ...
  function onDragHandleDown(e) {
    ...
    // Prefer the inline style value so we pick up prior drags correctly;
    // fall back to computed (which includes CSS-sheet values).
    const rawTop  = editingEl.style.marginTop  || cs.marginTop  || "0px";
    const rawLeft = editingEl.style.marginLeft || cs.marginLeft || "0px";
    dragState = {
      x: e.clientX,
      y: e.clientY,
      origMarginTop:  parseFloat(rawTop)  || 0,
      origMarginLeft: parseFloat(rawLeft) || 0,
      dragArmed: false,
    };
    const h = $("cf-drag-handle");
  ...
  function onDragMove(e) {
    ...
    editingEl.style.setProperty("margin-top",  (dragState.origMarginTop  + dy) + "px", "important");
    editingEl.style.setProperty("margin-left", (dragState.origMarginLeft + dx) + "px", "important");
    positionDragHandle();
    positionEditToolbar();
  }
  function onDragEnd() {
  ...
  function defaultBgState() {
    return {
      ...
      gradAngle: 180,
      // For radial gradients only: literal shape/size/position prefix string
      // (e.g. "95% 55% at 50% 112%"). Captured at edit start so radial geometry
      // round-trips on re-serialize. null when not a radial / no prefix present.
      radialPrefix: null,
      ...
      imgSize: "cover",
      imgPos: "center",
      // Pixel pan offsets layered on top of imgPos. When non-zero, applyBgState
      // overrides the keyword position with `calc(50% +/- panX px) calc(50% +/- panY px)`
      // so the wheel-pan gesture can fine-tune position without losing the
      // keyword-cycler affordance. Reset to 0 when the cycler picks a new keyword.
      panX: 0,
      panY: 0,
    };
  }
  ...
  function parseGradient(bgImage) {
    ...
    if (type === "linear") {
      ...
      // Capture optional shape/size/position prefix verbatim. Radial syntax:
      // `radial-gradient([<shape>] [<size>] [at <position>]?, <color-stop>, ...)`
      // The prefix ends at the first depth-0 comma whose RHS starts with a color
      // token. We preserve the literal prefix string (e.g. "95% 55% at 50% 112%")
      // and re-emit it on serialize so radial geometry round-trips losslessly.
      let pi = 0, depth = 0, prefixEnd = -1;
      while (pi < inner.length) {
        ...
        if (c === "(") depth++;
          ...
          let q = pi + 1;
          while (q < inner.length && /\s/.test(inner[q])) q++;
          if (readColorToken(inner, q)) { prefixEnd = pi; break; }
        }
        pi++;
      }
      if (prefixEnd > -1) {
        radialPrefix = inner.slice(0, prefixEnd).trim();
        inner = inner.slice(prefixEnd + 1).trim();
      }
    }
    ...
    return { type, angle: angle != null ? angle : 180, stops, radialPrefix };
  }

  function serializeGradient(type, angle, stops, radialPrefix) {
    ...
    return radialPrefix
      ? `radial-gradient(${radialPrefix}, ${stopStrs})`
      : `radial-gradient(${stopStrs})`;
  }
  ...
  function readBgStateFromElement(el) {
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      if (layer.startsWith("linear-gradient") || layer.startsWith("radial-gradient")) {
        ...
        if (g && !gradSeen) {
          ...
          state.stops = g.stops;
          state.radialPrefix = g.radialPrefix || null;
          gradSeen = true;
        }
      } else {
    ...
    // Parse calc-pair offsets if a prior wheel-pan was committed here, so
    // re-entering the editor restores the existing pan instead of snapping
    // back to keyword-center.
    const calcMatch = pos.match(/calc\(50%\s*([+-])\s*(\d+(?:\.\d+)?)px\)\s*calc\(50%\s*([+-])\s*(\d+(?:\.\d+)?)px\)/);
    if (calcMatch) {
      state.panX = parseFloat(calcMatch[2]) * (calcMatch[1] === "-" ? -1 : 1);
      state.panY = parseFloat(calcMatch[4]) * (calcMatch[3] === "-" ? -1 : 1);
    }
    return state;
  }
  ...
  function applyBgState() {
    ...
    // Decompose any inline `background:` shorthand into longhands before we
    // write our own longhands below. Without this, a shorthand with explicit
    // per-layer positions (e.g. `background: ..., url(...) center 70% / cover`)
    // gets serialized in cssText alongside our longhand `background-position`
    // and the cascade interaction can leave the shorthand's position in
    // effect for the image layer — making the position cycler look broken
    // even though our longhand IS being written. removeProperty clears it
    // outright; the longhands we set below (and always-write background-image)
    // fully reconstruct the bg.
    ...
    const grad = serializeGradient(bgState.gradType, bgState.gradAngle, bgState.stops, bgState.radialPrefix);
    ...
    // Always write background-image. We removed the shorthand above; without
    // explicitly writing this longhand the multi-layer bg-image would fall
    // back to whatever the stylesheet rule provides (often a single-layer URL),
    // losing the tint + gradient overlays.
    ...
    if (bgState.imgUrl) {
      ...
      // When wheel-pan offsets are non-zero, emit a calc-pair pos for the image
      // layer (overrides the keyword imgPos). Otherwise stick with the keyword
      // so the cycler's selection is the rendered value.
      const hasPan = (bgState.panX || 0) !== 0 || (bgState.panY || 0) !== 0;
      const imgPosCss = hasPan
        ? `calc(50% ${bgState.panX >= 0 ? "+" : "-"} ${Math.abs(bgState.panX).toFixed(2)}px) calc(50% ${bgState.panY >= 0 ? "+" : "-"} ${Math.abs(bgState.panY).toFixed(2)}px)`
        : bgState.imgPos;
      const positions = layerKinds.map((k) => k === "img" ? imgPosCss : "0% 0%");
  ...
  function ensureTexClass() {
    if (document.getElementById("cf-bs-tex-style")) return;
    const style = document.createElement("style");
    style.id = "cf-bs-tex-style";
    style.textContent =
      ".bs-tex { position: relative; }\n" +
      ".bs-tex::before { content: ''; position: absolute; inset: 0;" +
      " background-image: var(--tex-url, none); background-size: var(--tex-size, cover);" +
      " background-position: center; opacity: var(--tex-opacity, 1); z-index: 0; pointer-events: none; }";
  ...
  function populateBgToolbar() {
    ...
    const hasPan = (bgState.panX || 0) !== 0 || (bgState.panY || 0) !== 0;
    posBtn.textContent = hasPan
      ? `pos: ${Math.round(bgState.panX)},${Math.round(bgState.panY)}px`
      : "pos: " + bgState.imgPos;
    ...
    renderBgStops();
    // Texture rows: opacity row only when texture detected; URL row always visible.
    const texRow = $("cf-bg-row-tex");
    const texName = $("cf-bg-tex-name");
    const texUrlRow = $("cf-bg-row-tex-url");
    const texUrlLabel = texUrlRow ? texUrlRow.querySelector("label") : null;
    const texUrlApplyBtn = $("cf-bg-tex-url-apply");
    const texUrlClearBtn = $("cf-bg-tex-url-clear");
    if (bgEditingEl) {
      try {
        const beforeCs = window.getComputedStyle(bgEditingEl, "::before");
        const beforeImg = beforeCs ? beforeCs.backgroundImage : "";
        const m = beforeImg && beforeImg !== "none"
          ? beforeImg.match(/url\(["']?([^"')]+)["']?\)/)
          : null;
        if (m) {
          // Has texture — show name chip + opacity row, URL row in "sq url" mode.
          if (texName) {
            const parts = m[1].split("/");
            texName.textContent = parts[parts.length - 1] || m[1];
            texName.title = m[1];
          }
          if (texRow) texRow.style.display = "";
          const texOpSlider = $("cf-bg-tex-opacity");
          const texOpVal = $("cf-bg-tex-opacity-val");
          if (texOpSlider && texOpVal) {
            const raw = bgEditingEl.style.getPropertyValue("--tex-opacity").trim();
            const val = raw !== "" ? parseFloat(raw) : 1;
            texOpSlider.value = isNaN(val) ? 1 : val;
            texOpVal.textContent = (isNaN(val) ? 1 : val).toFixed(2);
          }
          if (texUrlRow) texUrlRow.style.display = "";
          if (texUrlLabel) texUrlLabel.textContent = "sq url";
          if (texUrlApplyBtn) texUrlApplyBtn.textContent = "apply";
          if (texUrlClearBtn) texUrlClearBtn.style.display = "";
          const texUrlInput = $("cf-bg-tex-url-input");
          if (texUrlInput) {
            const rawUrl = bgEditingEl.style.getPropertyValue("--tex-url").trim();
            const urlMatch = rawUrl.match(/url\(["']?([^"')]+)["']?\)/);
            texUrlInput.value = urlMatch ? urlMatch[1] : "";
          }
        } else {
          // No texture — hide opacity row, URL row in "add texture" mode.
          if (texRow) texRow.style.display = "none";
          if (texUrlRow) texUrlRow.style.display = "";
          if (texUrlLabel) texUrlLabel.textContent = "texture";
          if (texUrlApplyBtn) texUrlApplyBtn.textContent = "add";
          if (texUrlClearBtn) texUrlClearBtn.style.display = "none";
          const texUrlInput = $("cf-bg-tex-url-input");
          if (texUrlInput) texUrlInput.value = "";
        }
      } catch (e) {
        if (texRow) texRow.style.display = "none";
      }
  ...
  function exitBgEdit(keepEdit) {
    ...
    const noteInput = $("cf-bg-note"); if (noteInput) noteInput.value = "";
    bgEditingEl = null;
  ...
  function wireBgToolbar() {
    ...
    $("cf-bg-img-pos").addEventListener("click", (e) => {
      ...
      // Cycler picks reset any wheel-pan offset — the keyword IS the position.
      bgState.panX = 0;
      bgState.panY = 0;
    ...
    // Texture opacity slider — writes --tex-opacity onto the element inline so
    // the ::before rule picks it up via var(--tex-opacity, 1).
    $("cf-bg-tex-opacity").addEventListener("input", (e) => {
      e.stopPropagation();
      if (!bgEditingEl) return;
      const val = parseFloat(e.target.value);
      bgEditingEl.style.setProperty("--tex-opacity", val);
      const texOpVal = $("cf-bg-tex-opacity-val");
      if (texOpVal) texOpVal.textContent = val.toFixed(2);
    });
    $("cf-bg-tex-opacity").addEventListener("mousedown", (e) => e.stopPropagation());
    // Clear texture — removes whichever class is producing the ::before bg image.
    // Tests each class in turn: the one whose removal makes ::before.backgroundImage
    // go "none" is the responsible class. Fallback: zero --tex-opacity.
    $("cf-bg-tex-clear").addEventListener("click", (e) => {
      e.stopPropagation();
      if (!bgEditingEl) return;
      const snapshot = [...bgEditingEl.classList];
      let found = false;
      for (const cls of snapshot) {
        bgEditingEl.classList.remove(cls);
        let afterImg = "none";
        try {
          const cs = window.getComputedStyle(bgEditingEl, "::before");
          afterImg = cs ? (cs.backgroundImage || "none") : "none";
        } catch (_) {}
        if (afterImg === "none" || afterImg === "") {
          found = true; // leave cls removed
          break;
        }
        bgEditingEl.classList.add(cls); // not the one — restore
      }
      if (!found) {
        // Fallback: zero out opacity so it at least disappears visually.
        bgEditingEl.style.setProperty("--tex-opacity", "0");
        const texOpSlider = $("cf-bg-tex-opacity");
        const texOpVal = $("cf-bg-tex-opacity-val");
        if (texOpSlider) texOpSlider.value = 0;
        if (texOpVal) texOpVal.textContent = "0.00";
      }
      // Clean up all texture custom properties when a class was removed.
      if (found) {
        bgEditingEl.style.removeProperty("--tex-opacity");
        bgEditingEl.style.removeProperty("--tex-url");
        bgEditingEl.style.removeProperty("--tex-size");
      }
      populateBgToolbar();
    });
    $("cf-bg-tex-clear").addEventListener("mousedown", (e) => e.stopPropagation());
    // Texture URL override — writes --tex-url: url("...") onto the element so
    // the ::before background-image var() picks it up over the local fallback.
    $("cf-bg-tex-url-apply").addEventListener("click", (e) => {
      e.stopPropagation();
      if (!bgEditingEl) return;
      const url = $("cf-bg-tex-url-input").value.trim();
      if (!url) return;
      // If no texture class exists yet, inject .bs-tex to create one dynamically.
      let hasTexture = false;
      try {
        const beforeCs = window.getComputedStyle(bgEditingEl, "::before");
        const beforeImg = beforeCs ? beforeCs.backgroundImage : "";
        hasTexture = !!(beforeImg && beforeImg !== "none");
      } catch (_) {}
      if (!hasTexture) {
        ensureTexClass();
        bgEditingEl.classList.add("bs-tex");
        bgEditingEl.style.setProperty("--tex-opacity", "1");
      }
      bgEditingEl.style.setProperty("--tex-url", `url("${url}")`);
      populateBgToolbar();
    });
    $("cf-bg-tex-url-apply").addEventListener("mousedown", (e) => e.stopPropagation());
    $("cf-bg-tex-url-input").addEventListener("mousedown", (e) => e.stopPropagation());
    // Revert removes --tex-url so the CSS fallback (local file) takes over again.
    $("cf-bg-tex-url-clear").addEventListener("click", (e) => {
      e.stopPropagation();
      if (!bgEditingEl) return;
      bgEditingEl.style.removeProperty("--tex-url");
      $("cf-bg-tex-url-input").value = "";
      populateBgToolbar(); // refresh filename chip
    });
    $("cf-bg-tex-url-clear").addEventListener("mousedown", (e) => e.stopPropagation());
  }
  ...
  function setupSectionReorderControls() {
    ...
    setupSectionDividerDrag();
  }

  // ---------------- Section divider drag ----------------
  // Adds a 14px-tall transparent hit zone between consecutive sibling sections
  // with a 64x4 gold pill. Drag up/down to drive the UPPER section's
  // padding-bottom in real time — moves the boundary rather than creating a
  // gap below an unchanged section. On release, queues a `section-divider-drag`
  // comment. Handles are body-mounted to escape any section overflow:hidden.
  function findSectionGroup() {
    const all = Array.from(document.querySelectorAll("section"));
    if (!all.length) return null;
    const byParent = new Map();
    all.forEach((s) => {
      if (insideOurUI(s) || s.closest("[data-cf-ignore]")) return;
    ...
    byParent.forEach((sections) => {
      if (sections.length > bestCount) { best = sections; bestCount = sections.length; }
    });
    return best && best.length >= 2 ? best : null;
  }

  // Module-scoped state so the function is idempotent (callable on each section
  // reorder without leaking handles or listeners) and drag bookkeeping stays in
  // one place instead of per-divider mutation.
  let cfDividers = [];
  let cfDividerActive = null;     // currently-dragged divider or null
  let cfDividerRafPending = false;
  let cfDividerListenersBound = false;
  let cfDividerRo = null;

  function repositionDividers() {
    if (cfDividerRafPending) return;
    cfDividerRafPending = true;
    requestAnimationFrame(() => {
      cfDividerRafPending = false;
      for (let i = 0; i < cfDividers.length; i++) {
        const d = cfDividers[i];
        const rect = d.below.getBoundingClientRect();
        d.el.style.top = (rect.top + window.scrollY - 7) + "px";
      }
    });
  }

  function tearDownDividers() {
    cfDividers.forEach((d) => d.el.remove());
    cfDividers = [];
    cfDividerActive = null;
    if (cfDividerRo) { cfDividerRo.disconnect(); cfDividerRo = null; }
  }

  function setupSectionDividerDrag() {
    tearDownDividers();
    const sections = findSectionGroup();
    if (!sections) return;
    for (let i = 0; i < sections.length - 1; i++) {
      const above = sections[i], below = sections[i + 1];
      const d = document.createElement("div");
      d.className = "cf-divider-handle";
      d.setAttribute("data-cf-ignore", "");
      const bar = document.createElement("div");
      bar.className = "cf-divider-bar";
      bar.setAttribute("data-cf-ignore", "");
      d.appendChild(bar);
      document.body.appendChild(d);
      const rec = { el: d, above: above, below: below, startY: 0, startPadding: 0 };
      d.addEventListener("pointerdown", (e) => onDividerDown(e, rec));
      cfDividers.push(rec);
    }
    repositionDividers();
    if ("ResizeObserver" in window) {
      cfDividerRo = new ResizeObserver(repositionDividers);
      sections.forEach((s) => cfDividerRo.observe(s));
    }
    if (!cfDividerListenersBound) {
      window.addEventListener("scroll", repositionDividers, { passive: true });
      window.addEventListener("resize", repositionDividers);
      ...
      cfDividerListenersBound = true;
    }
  }

  function onDividerDown(e, rec) {
    if (e.button !== 0) return;
    e.preventDefault();
    cfDividerActive = rec;
    rec.startY = e.clientY;
    // Read computed padding as baseline so delta math stays consistent
    // regardless of whether the source-side padding was inline or CSS.
    rec.startPadding = parseFloat(window.getComputedStyle(rec.above).paddingBottom) || 0;
    rec.el.classList.add("cf-dragging");
    try { rec.el.setPointerCapture(e.pointerId); } catch (_) { /* older browsers */ }
    document.body.style.userSelect = "none";
    document.body.style.cursor = "ns-resize";
  }

  function onDividerMove(e) {
    const rec = cfDividerActive;
    if (!rec) return;
    const delta = e.clientY - rec.startY;
    const next = Math.max(0, rec.startPadding + delta);
    rec.above.style.setProperty("padding-bottom", next + "px", "important");
    repositionDividers();
  }
  ...
  function onDividerUp() {
    ...
    if (!rec) return;
    cfDividerActive = null;
    rec.el.classList.remove("cf-dragging");
    document.body.style.userSelect = "";
    document.body.style.cursor = "";
    const newPadding = parseFloat(window.getComputedStyle(rec.above).paddingBottom) || 0;
    const newRound = Math.round(newPadding), oldRound = Math.round(rec.startPadding);
    if (newRound === oldRound) return;
    window.cfFeedback.queueComment({
      type: "section-divider-drag",
      above_id: rec.above.id || null,
      below_id: rec.below.id || null,
      new_padding_bottom_px: newRound,
      old_padding_bottom_px: oldRound,
      comment: "Dragged divider between #" + (rec.above.id || "?") + " and #" + (rec.below.id || "?") + " — padding-bottom on the upper section now " + newRound + "px (was " + oldRound + "px).",
    });
  }
  ...
  function reorderSection(sec, direction) {
    ...
    updateReorderButtonStates();
    // Rebuild divider handles — their `above`/`below` refs are now stale.
    setupSectionDividerDrag();
    pending.push({
      id: newCommentId(),
      type: "section-reorder",
  ...
  function exitEditMode(keepVisualEdit) {
    ...
    editingEl.classList.remove("cf-editing-target", "cf-editing-image", "cf-editing-drag");
  ...
  function submitTextEdit() {
    ...
    // Emptied → delete: if the user wiped all text from a text-bearing
    // element, queue a `delete` instead of a text-edit with empty content.
    // Skip when refining an already-pending edit (different intent).
    const becameEmpty = textChanged
      && (editingOriginalText || "").trim() !== ""
      && newText.trim() === ""
      && !refiningPendingId
      && editingEl.parentElement;
    if (becameEmpty) {
      const parent = editingEl.parentElement;
      const index = Array.from(parent.children).indexOf(editingEl);
      pending.push({
        id: "c-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6) + "-0",
        type: "delete",
        created_at: new Date().toISOString(),
        element: anchorInfo(editingEl),
        parent: {
          tag: parent.tagName.toLowerCase(),
          id: parent.id || null,
          selector: stableSelector(parent),
      ...
      renderPending();
      exitEditMode(true);
      showToast("emptied — queued as delete", 2400);
      return;
    }
  ...
  function openColorPopover(targetProp, anchorEl) {
    ...
    const _transparentBtn = $("cf-color-popover-transparent");
    if (_transparentBtn) _transparentBtn.style.display = targetProp === "background-color" ? "" : "none";
    renderPalette();
  ...
  function applyInlineStyle(prop, value) {
    ...
    // Scrub nested same-property inline overrides. Without this, element-level
    // changes silently lose to any descendant <span style="prop: …!important">
    // left behind by prior selection-scoped edits — and the user sees "font-size
    // doesn't work." Skip element-only props (no descendant span would carry
    // them anyway).
    if (value != null && value !== "" && !ELEMENT_LEVEL_ONLY.has(prop)) {
      const all = editingEl.querySelectorAll("[style]");
      for (let i = 0; i < all.length; i++) {
        const desc = all[i];
        if (desc === editingEl) continue;
        if (desc.style.getPropertyValue(prop)) {
          desc.style.removeProperty(prop);
          if (!desc.style.cssText) desc.removeAttribute("style");
        }
      }
    }
    set(editingEl);
  ...
  function applyZoomedPan(el, x, y) {
    ...
    // WebKit (iOS Safari & all iOS browsers) ignores object-view-box. Also
    // emit CSS custom properties so a page-level @supports rule can emulate
    // the crop via `transform: scale(var(--cf-zoom)) translate(var(--cf-pan-tx), var(--cf-pan-ty))`.
    // The translate uses the same panX/panY-as-%-of-natural-dim as the view-box
    // inset asymmetry; sign convention matches (positive panX shifts image
    // right in frame). The page is responsible for the @supports rule and
    // ensuring the img has an overflow:hidden ancestor.
    const panXPctSigned = ((x / nw) * 100).toFixed(3);
    const panYPctSigned = ((y / nh) * 100).toFixed(3);
    el.style.setProperty("--cf-zoom", String(zoom));
    el.style.setProperty("--cf-pan-tx", panXPctSigned + "%");
    el.style.setProperty("--cf-pan-ty", panYPctSigned + "%");
    return true;
  }
  ...
  function applyVAlign(el, valign) {
    if (!el) return;
    // Clear stale align-self / vertical-align from earlier write paths. Do
    // NOT touch margin-top / margin-bottom — those are dimensional pins the
    // user sets via the resize handle and have nothing to do with valign now
    // that the implementation uses display:flex + justify-content.
  ...
  function populateStyleControls(el) {
    ...
    // Effect classes — read the first matching cf-fx-* / cf-bxfx-* class
    // from the element and set each select to it. Empty if no effect applied.
    const fxSel = $("cf-edit-effect");
    if (fxSel) {
      const fxClass = Array.from(el.classList).find(c => c.startsWith("cf-fx-")) || "";
      fxSel.value = fxClass;
    }
    const bxFxSel = $("cf-edit-boxfx");
    if (bxFxSel) {
      const bxClass = Array.from(el.classList).find(c => c.startsWith("cf-bxfx-")) || "";
      bxFxSel.value = bxClass;
    }
  }
  ...
  const CF_CURATED_FONTS = [
    ...
    // Brand fonts — loaded by the @import at the top of feedback.css so they
    // render correctly on any host page, even when the page itself doesn't
    // declare them.
    ['"Montserrat", system-ui, sans-serif',                                'Montserrat',         'sans'],
    ['"Josefin Sans", Helvetica, Arial, sans-serif',                       'Josefin Sans',       'sans'],
    ['"Inter", system-ui, sans-serif',                                     'Inter',              'sans'],
    ['"Lato", Helvetica, Arial, sans-serif',                               'Lato',               'sans'],
    ['"Poppins", system-ui, sans-serif',                                   'Poppins',            'sans'],
    ['"Public Sans", system-ui, sans-serif',                               'Public Sans',        'sans'],
    ['"Mulish", system-ui, sans-serif',                                    'Mulish',             'sans'],
    ['"Alegreya Sans SC", Verdana, Geneva, sans-serif',                    'Alegreya Sans SC',   'sans'],
    ['"Marcellus", Georgia, serif',                                        'Marcellus',          'serif'],
    ['"Cormorant Garamond", Georgia, serif',                               'Cormorant Garamond', 'serif'],
    ['"Lora", Georgia, serif',                                             'Lora',               'serif'],
    ['"Fraunces", Georgia, serif',                                         'Fraunces',           'serif'],
    ['"Source Serif 4", Georgia, serif',                                   'Source Serif 4',     'serif'],
    ['"Pinyon Script", "Cormorant Garamond", cursive',                     'Pinyon Script',      'script'],
    // System / web-safe — work without any font load.
  ...
  function buildFontFamilyOptions() {
    ...
    // Curated by category
    const groups = { sans: "Sans-serif", serif: "Serif", script: "Script", mono: "Monospace" };
  ...
  async function fetchPrompts() {
    try {
      // cache: "no-cache" — always revalidate with If-Modified-Since. Server's
      // SimpleHTTPRequestHandler sends Last-Modified, so unchanged polls get
      // a 304 (no body transfer) and the fetch returns the cached response.
      // Previously we cache-busted with ?t=Date.now(); that defeated 304 and
      // forced a full re-download every POLL_INTERVAL_MS.
      const resp = await fetch(PROMPTS_URL, { cache: "no-cache" });
      if (!resp.ok) {
  ...
  async function fetchHistory() {
    try {
      const resp = await fetch(HISTORY_URL, { cache: "no-cache" });
      if (!resp.ok) return;
      const text = await resp.text();
  ...
  function bindEvents() {
    ...
    $("cf-elem-popup-parent").addEventListener("click", () => {
      if (selectedElements.length === 0) return;
      const last = selectedElements[selectedElements.length - 1];
      const parent = findParentCommentable(last);
      if (!parent) return;
      selectedElements.forEach(x => { x.classList.remove("cf-elem-selected"); removeSelectionFrame(x); });
      selectedElements = [];
      selectedElements.push(parent);
      parent.classList.add("cf-elem-selected");
      parent.classList.remove("cf-elem-hover");
      addSelectionFrame(parent);
      setHoverFrame(null);
      showElemPopup(parent);
    });
    $("cf-elem-popup-resize").addEventListener("click", () => {
      if (selectedElements.length !== 1) return;
      const el = selectedElements[0];
      // Exit element mode first (startImageEdit bails if elementMode is active)
      if (elementMode) toggleElementMode();
      startImageEdit(el);
    });
    $("cf-elem-popup-move").addEventListener("click", () => {
      if (selectedElements.length !== 1) return;
      const el = selectedElements[0];
      if (elementMode) toggleElementMode();
      startDragEdit(el);
    });
    $("cf-drag-handle").addEventListener("pointerdown", onDragHandleDown);
    ...
    // Mirror — toggle a horizontal flip on the image. The state is held in the
    // element's inline transform, so the outerHTML diff captures it on confirm
    // and the agent persists it as style="transform: scaleX(-1) !important;".
    // Compose-safe: if the user has another transform applied (rotation, etc.),
    // we strip the scaleX token and re-emit it, leaving sibling transform
    // functions intact. Two clicks return the image to its original orientation
    // and remove the inline transform entirely.
    $("cf-edit-img-mirror").addEventListener("click", () => {
      if (!editingEl || editingEl.tagName !== "IMG") return;
      const cur = editingEl.style.getPropertyValue("transform") || "";
      const stripped = cur.replace(/\bscaleX\(-1\)/g, "").replace(/\s+/g, " ").trim();
      const wasMirrored = stripped !== cur;
      if (wasMirrored) {
        if (stripped) {
          editingEl.style.setProperty("transform", stripped, "important");
        } else {
          editingEl.style.removeProperty("transform");
        }
      } else {
        const next = stripped ? stripped + " scaleX(-1)" : "scaleX(-1)";
        editingEl.style.setProperty("transform", next, "important");
      }
    });
    ...
    $("cf-color-popover-transparent").addEventListener("mousedown", (e) => e.preventDefault());
    $("cf-color-popover-transparent").addEventListener("click", () => {
      applyInlineStyle("background-color", "");
      syncColorSwatchButtons();
      closeColorPopover();
    });
    ...
    const fxSelEl = $("cf-edit-effect");
    if (fxSelEl) {
      fxSelEl.addEventListener("mousedown", (e) => e.stopPropagation());
      fxSelEl.addEventListener("change", (e) => {
        if (!editingEl) return;
        Array.from(editingEl.classList).forEach(c => { if (c.startsWith("cf-fx-")) editingEl.classList.remove(c); });
        const v = e.target.value;
        if (v) editingEl.classList.add(v);
      });
    }
    const bxFxSelEl = $("cf-edit-boxfx");
    if (bxFxSelEl) {
      bxFxSelEl.addEventListener("mousedown", (e) => e.stopPropagation());
      bxFxSelEl.addEventListener("change", (e) => {
        if (!editingEl) return;
        Array.from(editingEl.classList).forEach(c => { if (c.startsWith("cf-bxfx-")) editingEl.classList.remove(c); });
        const v = e.target.value;
        if (v) editingEl.classList.add(v);
      });
    }
    ...
    document.addEventListener("wheel", (e) => {
      // BG-edit pan: scroll over the bg-editing element to slide its bg image.
      // Mirrors the image-edit wheel pan below — scroll down moves the photo
      // up, scroll right moves it left. Writes calc-pair offsets into
      // bgState.panX/panY which applyBgState renders as a calc-pair pos.
      if (bgEditingEl && document.body.classList.contains("cf-bg-edit-mode")) {
        if (insideOurUI(e.target)) return;
        if (e.target !== bgEditingEl && !bgEditingEl.contains(e.target)) return;
        if (!bgState || !bgState.imgUrl) return;  // no image → nothing to pan
        e.preventDefault();
        let dxB = e.deltaX, dyB = e.deltaY;
        if (e.deltaMode === 1) { dxB *= 16; dyB *= 16; }
        else if (e.deltaMode === 2) { dxB *= 100; dyB *= 100; }
        // Positive pan shifts the photo right/down on screen; scroll-right /
        // scroll-down conventionally moves the viewport right/down which is
        // the photo moving LEFT/UP relative to the frame → subtract.
        bgState.panX = (bgState.panX || 0) - dxB;
        bgState.panY = (bgState.panY || 0) - dyB;
        applyBgState();
        const posBtn = $("cf-bg-img-pos");
        if (posBtn) {
          posBtn.textContent = `pos: ${Math.round(bgState.panX)},${Math.round(bgState.panY)}px`;
        }
        return;
      }
      if (!editingEl) return;
    ...
    // Throttle through rAF — without this, every scroll tick re-reads offsetHeight/Width
    // and re-clamps left/top, causing visible jitter on mobile (address bar collapse +
    // arrow-key caret scroll both fire scroll events many times per gesture).
    let cfScrollRaf = 0;
    window.addEventListener("scroll", () => {
      if (cfScrollRaf) return;
      cfScrollRaf = requestAnimationFrame(() => {
        cfScrollRaf = 0;
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (editingEl && document.body.classList.contains("cf-image-edit-mode") &&
        ...
        if (e.key === "+" || e.key === "=" || e.key === "-") {
          ...
          // Compose-safe migration off the legacy clip-path + transform-scale
          // zoom approach (the new one uses object-view-box). Strip ONLY the
          // legacy zoom-shaped tokens: a scale(N) function from transform,
          // and a clip-path whose value is just an inset() (the shape the old
          // zoom emitted). Leave any user-intended translateY/rotate/etc and
          // non-inset clip-paths alone — those are not vestigial zoom state.
          const tr = editingEl.style.getPropertyValue("transform") || "";
          if (tr) {
            const stripped = tr.replace(/\bscale\s*\([^)]*\)/g, "").replace(/\s+/g, " ").trim();
            if (stripped) editingEl.style.setProperty("transform", stripped, "important");
            else editingEl.style.removeProperty("transform");
          }
          const cp = editingEl.style.getPropertyValue("clip-path") || "";
          if (/^inset\s*\(/i.test(cp.trim())) editingEl.style.removeProperty("clip-path");
          // transform-origin is only meaningful with a transform; if we just
          // stripped the only token, drop the origin too. Otherwise leave it.
          if (!editingEl.style.getPropertyValue("transform")) {
          ...
          if (scale === 1) {
            ...
            // Also drop the WebKit-fallback CSS vars
            editingEl.style.removeProperty("--cf-zoom");
            editingEl.style.removeProperty("--cf-pan-tx");
            editingEl.style.removeProperty("--cf-pan-ty");
      ...
      switch (e.key) {
        ...
        case "r": case "R":
          ...
          break;
        case "l": case "L":
          e.preventDefault(); copyPageHtml(); break;
      }
    });
  }
