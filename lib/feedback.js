...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    </div>',
      '    <p class="cf-hint"><strong>Highlight text</strong> to comment on a selection. <strong>Double-click text</strong> to edit it in place — toolbar has bold/italic, lists, links, alignment, case, font, color, border, radius, padding. <strong>Double-click an image</strong> to drag its corners and resize (border + radius stay available). Press <kbd>E</kbd> to select elements (then comment or <strong>delete</strong>), <kbd>M</kbd> to drag-reorder, <kbd>G</kbd> for a general note. Hold <kbd>Alt</kbd> + drag a rectangle to <strong>snapshot a region</strong>. Toggle a 24 px grid + snap from the panel. Press <kbd>?</kbd> for the full guide. <kbd>Esc</kbd> cancels.</p>',
      '  </div>',
  ...
  function resetInlineStyles() {
    if (!editingEl) return;
    ["font-family", "font-size", "color", "background-color", "border", "border-radius", "padding"]
      .forEach((p) => {
  ...
  function applyCaseTransform(mode) {
    if (!editingEl) return;
    // Neutralize CSS text-transform on the editing element so the new case
    // is actually visible — without this, a class like `.card-min { text-
    // transform: uppercase }` keeps rendering uppercase even after the
    // source has been switched to lower/title, and the button looks dead.
    // The override lands inline + !important so it beats any selector.
    const cts = getComputedStyle(editingEl).textTransform;
    if (cts && cts !== "none") {
