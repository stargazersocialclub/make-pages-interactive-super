...
(function () {
  ...
  function sqsRequestScopeWrap(rootId) {
    ...
    const batch = {
      ...
      source: "sqs-audit-scope-request",
      comments: [{
  ...
  function sqsApplySelected() {
    ...
    // Direct POST to /feedback — bypass pending + submitBatch entirely.
    const batch = {
      submitted_at: new Date().toISOString(),
      page_url: location.pathname,
      source: "sqs-audit",
  ...
  function onDblClick(e) {
    ...
    // Fallback: walk-up missed it (e.g. a wrapper div with text intercepted
    ...
    // node in that stack. Fixes the long-standing "double-click an image
    // opens the text editor" report — common when an <a> wraps an <img>, when
    // a flex sibling overlaps the image, or when the wrapper has its own
    // text content that wins on the walk-up.
