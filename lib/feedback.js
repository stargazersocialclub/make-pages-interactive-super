...
(function () {
  ...
  const SQS_NAKED_TAGS = new Set([
    "body", "html", "*",
    "h1", "h2", "h3", "h4", "h5", "h6",
    "p", "a", "button", "input", "select", "textarea", "form",
  ...
  function sqsDetectNakedTags(blocks) {
    ...
    blocks.forEach((block, blockIdx) => {
      ...
      sels.forEach(({ selector }) => {
        ...
        const head = selector.split(/[\s>+~]/)[0].trim();
        if (!SQS_GLOBAL_TAGS.has(head) && SQS_NAKED_TAGS.has(head)) {
          found.push({ blockIdx, selector, head });
        }
      });
    });
  ...
  function sqsDetectFooterCollision() {
    const found = [];
    ...
    document.querySelectorAll("*").forEach((el) => {
      ...
      // Skip elements with `bottom: auto` (positioned via top)
      const bottom = parseFloat(cs.bottom);
      if (!isFinite(bottom)) return;
      if (bottom < 0 || bottom > 60) return;
      // Element sits near the bottom; could collide with Squarespace footer
      const r = el.getBoundingClientRect();
      if (r.bottom > vh - 5) {
        found.push({ el, bottom });
      }
    });
    return found;
  }

  function sqsDetectViewportUnits(blocks) {
    const found = [];
    blocks.forEach((block, blockIdx) => {
      const text = block.textContent;
      const re = /(\d+)(vw|vh)\b/g;
      let m;
      while ((m = re.exec(text)) !== null) {
        if (parseInt(m[1], 10) >= 50) {
          found.push({ blockIdx, match: m[0], unit: m[2] });
        }
      }
    });
    return found;
  ...
  function sqsCollectFindings() {
    ...
    return {
      ...
      inline: sqsDetectInlineNoImportant(),
      footer: sqsDetectFooterCollision(),
      viewport: sqsDetectViewportUnits(blocks),
    };
  }

  // Guess a sensible default root id for scoping. Prefer the outermost div
  // with an id under body that wraps the page content.
  function sqsGuessRootId() {
    const candidates = document.body.querySelectorAll("[id]");
    for (const el of candidates) {
      if (el.closest("#claude-feedback-root")) continue;
      const id = el.id;
      if (!id || id.startsWith("cf-")) continue;
      // Prefer ids that look like page roots
      if (/root|page|app|main|wrap|container/.test(id.toLowerCase())) return id;
    }
    // Fall back to the first non-cf id
    for (const el of candidates) {
      if (el.closest("#claude-feedback-root")) continue;
      if (!el.id || el.id.startsWith("cf-")) continue;
      return el.id;
    }
    return "";
  }
  ...
  function runSquarespaceAudit() {
    ...
    const total = findings.global.length + findings.naked.length +
                  findings.inline.length + findings.footer.length +
                  findings.viewport.length;
    if (total === 0) {
  ...
  function openSqsModal(findings) {
    ...
    const parts = [];
    parts.push(`<p class="cf-sqs-intro">Five checks ran against this page. Tick a finding to queue its fix as a pending text-edit; leave it unticked to skip. Two checks need a decision before fixes apply (root id, footer lift).</p>`);

    // Critical decisions surface up top
    parts.push(`<div class="cf-sqs-decision">`);
    parts.push(`  <label class="cf-sqs-decision-label">Root id to scope under <small>(used by global + naked-tag fixes)</small></label>`);
    parts.push(`  <input type="text" id="cf-sqs-root" value="${escapeHtml(guess)}" placeholder="e.g. ssc-pricing-root" class="cf-sqs-input">`);
    parts.push(`</div>`);
    parts.push(`<div class="cf-sqs-decision">`);
    parts.push(`  <label class="cf-sqs-decision-label">Footer lift offset (px) <small>(applied to position:fixed elements colliding with the Squarespace footer)</small></label>`);
    parts.push(`  <input type="number" id="cf-sqs-lift" value="80" min="60" max="200" step="4" class="cf-sqs-input">`);
    parts.push(`</div>`);
    ...
    const section = (name, label, sev, items, render) => {
      ...
      items.forEach((item, i) => {
        ...
        parts.push(`    <div class="cf-sqs-item-text">${render(item)}</div>`);
        parts.push(`  </label>`);
      });
      parts.push(`</div>`);
    };
    ...
    section("naked", "Naked tag selectors — collide with Squarespace typography", "warning", findings.naked, (f) =>
      `<code>${escapeHtml(f.selector)}</code> &mdash; prepend <code>#&lt;rootId&gt; </code>`
    );
    section("inline", "Inline styles missing <code>!important</code>", "info", findings.inline, (f) =>
      `<code>&lt;${escapeHtml(f.el.tagName.toLowerCase())}&gt;</code>: <code>${escapeHtml(f.prop)}: ${escapeHtml(f.val)}</code>`
    );
    section("footer", "<code>position: fixed</code> elements colliding with the Squarespace footer", "critical", findings.footer, (f) =>
      `<code>&lt;${escapeHtml(f.el.tagName.toLowerCase())}.${escapeHtml(f.el.className.split(/\s+/).filter(Boolean)[0] || "")}&gt;</code> at <code>bottom: ${Math.round(f.bottom)}px</code>`
    );
    section("viewport", "Viewport units (<code>100vw</code> / <code>100vh</code>) — overflow Squarespace's constrained container", "warning", findings.viewport, (f) =>
      `<code>${escapeHtml(f.match)}</code> — convert to <code>100%</code>`
    );
  ...
  // Apply the user's selections: each checked item turns into a pending
  // text-edit comment. Style-block fixes target the host <style> element
  // and rewrite its textContent.
  function sqsApplySelected() {
    ...
    const lift = parseInt($("cf-sqs-lift").value, 10) || 80;
    const blocks = sqsCollectStyleBlocks();
    let queued = 0;

    // Group style-block changes per block so multiple selector rewrites in
    // the same <style> share one text-edit entry.
    ...
    // 1 + 2: rewrite selectors in <style> blocks
    const scopeFixes = [];
    if (rootId) {
    ...
    scopeFixes.forEach((f) => {
      ...
      // Replace bare head-token at start of rule openings.
      // Match the selector followed by `{` or `,`, preceded by `}`, `;`, or start.
      const escHead = head.replace(/[*]/g, "\\*");
      let next;
      if (head === "body" || head === "html") {
        // Replace `body { ... }` → `#root { ... }`; replace `body.foo` → `#root .foo`
        next = cur.replace(new RegExp("(^|[\\s\\},;])" + escHead + "(?=[\\s\\{,.\\[#:>+~])", "gm"), "$1#" + rootId);
      } else if (head === "*") {
        next = cur.replace(/(^|[\s\},;])\*(?=[\s\{,])/gm, "$1#" + rootId + " *");
      } else {
        // h1, p, etc. — prepend `#rootId ` so it becomes a descendant rule
        next = cur.replace(new RegExp("(^|[\\s\\},;])" + escHead + "(?=[\\s\\{,.\\[#:>+~])", "gm"), "$1#" + rootId + " " + head);
      }
      setBlockText(f.blockIdx, next);
    });

    // 3: inline styles — add !important to flagged decls
    findings.inline.forEach((f, i) => {
      ...
      const new_outer_html = f.el.outerHTML;
      pending.push({
        id: "c-sqs-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
        type: "text-edit",
        created_at: new Date().toISOString(),
        comment: "Squarespace audit: add !important to inline " + f.prop,
        elements: [anchorInfo(f.el)],
        original_text: f.el.innerText || "",
        new_text: f.el.innerText || "",
        original_html: f.el.innerHTML,
        new_html: f.el.innerHTML,
        original_outer_html,
        new_outer_html,
      });
      queued++;
    ...
    // 4: footer collision — set bottom: liftPx !important via inline style
    findings.footer.forEach((f, i) => {
      if (!checked("footer", i)) return;
      const original_outer_html = f.el.outerHTML;
      f.el.style.setProperty("bottom", lift + "px", "important");
      const new_outer_html = f.el.outerHTML;
      pending.push({
        id: "c-sqs-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
        type: "text-edit",
        created_at: new Date().toISOString(),
        comment: "Squarespace audit: lift position:fixed above footer (" + lift + "px)",
        elements: [anchorInfo(f.el)],
        original_text: f.el.innerText || "",
        new_text: f.el.innerText || "",
        original_html: f.el.innerHTML,
    ...
    // 5: viewport units — rewrite 100vw / 100vh → 100% in their style blocks
    findings.viewport.forEach((f, i) => {
      if (!checked("viewport", i)) return;
      const cur = getBlockText(f.blockIdx);
      const next = cur.replace(new RegExp(f.match.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), "100%");
      setBlockText(f.blockIdx, next);
    });

    // Commit style-block edits as text-edits on the <style> element
    blockEdits.forEach((newText, idx) => {
      ...
      const new_outer_html = styleEl.outerHTML;
      pending.push({
        id: "c-sqs-" + Date.now() + "-" + Math.random().toString(36).slice(2, 6),
        type: "text-edit",
        created_at: new Date().toISOString(),
        comment: "Squarespace audit: rewrote CSS in <style> block #" + (idx + 1),
        elements: [anchorInfo(styleEl)],
        original_text: oldText,
        new_text: newText,
        original_html: oldText,
        new_html: newText,
        original_outer_html,
        new_outer_html,
      });
    ...
    showToast(queued === 0
      ? "no fixes queued"
      : "Squarespace audit: queued " + queued + " fix" + (queued === 1 ? "" : "es"),
      4000);
  }
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    <button id="cf-sqs-audit" class="cf-btn cf-btn-small cf-sqs-audit-btn" title="Squarespace conflict audit — scans CSS + layout for Code-Injection issues and queues fixes" aria-label="Squarespace audit">🔍 sqs</button>',
      ...
      '        <ul>',
      '          <li>The <strong>🔍 sqs</strong> button next to copy-HTML runs a five-check audit for Squarespace Code Injection conflicts: global CSS selectors (<code>body</code> / <code>html</code> / <code>*</code>), naked tag selectors (<code>h1</code>, <code>p</code>, <code>a</code>, etc.), inline styles missing <code>!important</code>, <code>position: fixed</code> elements overlapping the Squarespace footer, and <code>100vw</code> / <code>100vh</code> sizing.</li>',
      '          <li>The modal asks for a <strong>root id</strong> to scope selectors under (auto-detected from your page) and a <strong>footer lift offset</strong> (default 80 px). Per-item checkboxes let you skip any finding. Apply queues each fix as a pending text-edit — they go through the standard submit / revert flow.</li>',
      '        </ul>',
      '      </section>',
      '      <section>',
      '        <h4>Launcher</h4>',
      ...
      '    <label class="cf-style-lbl">line</label>',
